import storageDialog_default from "./storageDialog.js";
//#region src/editor/extensions/ext-storage/storageDialog.js
var template = document.createElement("template");
template.innerHTML = storageDialog_default;
/**
* @class SeStorageDialog
*/
var SeStorageDialog = class extends HTMLElement {
	/**
	...
	*/
	static get observedAttributes() {
		return [
			...
			"common-ok",
			"common-cancel",
			"notify-editor_pref_msg",
			"properties-prefs_and_content",
			"tools-remember_this_choice",
			"tools-remember_this_choice_title",
			"properties-prefs_only",
			"properties-no_prefs_or_content"
		];
	}
	/**
	...
	*/
	attributeChangedCallback(name, oldValue, newValue) {
		...
		switch (name) {
			case "dialog":
				if (newValue === "open") this.$dialog.open();
				else this.$dialog.close();
				break;
			case "storage":
				if (newValue === "true") this.$storageInput.options[0].disabled = false;
				else this.$storageInput.options[0].disabled = true;
				break;
			case "common-ok":
...
//#endregion
export { SeStorageDialog };

//# sourceMappingURL=storageDialog2.js.map